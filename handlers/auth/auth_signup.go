...
func (h *Handler) AuthSignUp(c echo.Context) error {
	...
	exist, err := h.Mapper.FindOne(ctx, filter, &users.User{})
	if err != nil {
	...
	newUser := users.NewUser(body.Email, body.Username)
	...
	user, err := h.Mapper.FindOneAndUpdate(ctx, filter, newUser, &users.User{}, opts)
	if err != nil {
	...
	return h.Validate(c, http.StatusOK, user.(*users.User).Response())
}
